...
%-------------------------------------------------------------------------%
function [z,V,E,fem,f,g1,Change1,Optf1, u_v_a] = PolyDyna(fem,opt)
...
while (Iter<opt.MaxIter) && (max(Change)>Tol) %Optimization iterations
  ...
  [f,dfdz,fem, u_v_a] = ObjectiveFnc(fem,E,V,dEdy,dVdy,P);
...
g1 = max(g); Change1 = max(Change); Optf1 = f/fem.SF;
%------------------------------------------------------- OBJECTIVE FUNCTION
function [f,dfdz,fem, u_v_a] = ObjectiveFnc(fem,E,V,dEdy,dVdy,P)
